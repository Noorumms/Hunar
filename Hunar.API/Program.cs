...
builder.Services.AddScoped<CreateProviderProfileUseCase>(); // ADD THIS
builder.Services.AddScoped<GetProvidersUseCase>();          // ADD THIS
builder.Services.AddScoped<AddServiceListingUseCase>();     // ADD THIS
...
// ?? CUSTOM MIDDLEWARE ?????????????????????????????????????????????
// ExceptionMiddleware must be FIRST so it catches all errors
// including ones thrown by other middleware
